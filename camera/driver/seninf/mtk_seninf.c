...
static int seninf_parse_fwnode(struct device *dev, struct v4l2_async_notifier *notifier) {
	...
	fwnode_graph_for_each_endpoint(dev_fwnode(dev), fwnode) {
		...
		if (vep.base.port >= NUM_SENSORS)
			continue;

		if (vep.bus_type != V4L2_MBUS_CSI2_DPHY) {
			dev_err(dev, "Only CSI2 bus type is currently supported, port:%d\n",
				vep.base.port);
			continue;
		}

		s_asd = v4l2_async_nf_add_fwnode_remote(notifier, fwnode, struct sensor_async_subdev);
		if (IS_ERR(s_asd)) {
			ret = PTR_ERR(s_asd);
			break;
		}
...
static int mtk_seninf_media_register(struct mtk_seninf *priv)
{
	...
	ret = seninf_parse_fwnode(dev, &priv->notifier);
	if (ret < 0)
