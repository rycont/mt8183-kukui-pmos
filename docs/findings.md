...
## 13. AF VCM 은 동작한다 — 다만 최단 초점거리 밖에서는 확인할 수 없다

dw9768 을 DT 에 붙이면 `V4L2_CID_FOCUS_ABSOLUTE` (0..1023) 가 생기고
렌즈가 실제로 움직인다. `focus=0` 이 무한대, `1023` 이 최근접이다.

이걸 확인하는 데 시간을 썼다. 처음엔 렌즈가 안 움직이는 줄 알았다.
전체 프레임 선명도가 초점에 따라 7% 밖에 안 변했기 때문이다. 원인은
장면이었다 — 가까운 피사체가 화면을 채우고 있었고 그게 민무늬였다.

프레임을 두 영역으로 나눠 재니 정반대 곡선이 나왔다.

| focus | 원경(배경 책장) | 중앙(근접 피사체) |
|---|---|---|
| 0 | **0.1662** | 0.0919 |
| 200 | 0.1638 | 0.0925 |
| 500 | 0.1295 | 0.1033 |
...
배경은 0 에서 정점을 찍고 내려가고(정상), 근접 피사체는 1023 까지 정점
없이 계속 오른다. **정점 없는 단조 증가는 피사체가 최단 초점거리보다
가깝다는 신호다.** 그 거리에서는 어떤 초점 위치도 맞지 않는다.

교훈 두 가지. 대비검출 AF 의 측정창에는 무늬가 있어야 한다 — 민무늬
영역에서는 곡선이 평평해져 노이즈를 피크로 고른다. 그리고 위치당
프레임 수가 부족하면(6 장) AE 잔여 변동이 초점 차이보다 커서 결과가
무의미해진다. 소프트웨어 AGC 는 수렴에 20 프레임 가까이 쓴다.
